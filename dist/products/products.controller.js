...
let ProductsController = class ProductsController {
    ...
    async searchProducts(name, response) {
        if (!name) {
            response.status(common_1.HttpStatus.BAD_REQUEST).json({ ok: false, result: [], msg: 'Name query parameter is required' });
            return;
        }
        const result = await this.service.findByName(name);
        response.status(common_1.HttpStatus.OK).json({ ok: true, result, msg: 'Search Results' });
    }
};
...
__decorate([
    ...
    __metadata("design:paramtypes", [products_dto_1.ProductDto, Object]),
